...
namespace zhengwang1201\cronjobs;
...
RAW;
     ...
     */
    protected function parseSettings($runSettings)
    {
        if (empty($runSettings)) return array();

        //cron, cron-tags, cron-args, cron-strout, cron-stderr
        if (array_key_exists($this->tagPrefix, $runSettings))
            $return[0] = $runSettings[$this->tagPrefix];
        else
            throw new Exception('Settings error');

        foreach (['stdout', 'stderr', 'args', 'tags'] as $item) {
            $index = $this->tagPrefix . '-' . $item;
            array_key_exists($index, $runSettings) && $return[$item] = $runSettings[$index];
        }
            ...
            foreach ($methods as $runCommand => $runSettings) {
                ...
                if (count($runCommand) == 2) {
                    $actions[] = array(
                        ...
                        'action' => $runCommand[1],
                        'docs' => $this->parseSettings($runSettings)
                    );
                }
